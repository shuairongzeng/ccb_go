...
// GeminiAdapter implements the Adapter interface for Gemini.
type GeminiAdapter struct {
	BaseAdapter
	Backend   terminal.Backend
	Comm      *comm.GeminiCommunicator
	lastReply string
}

func NewGeminiAdapter(backend terminal.Backend) *GeminiAdapter {
...
func (a *GeminiAdapter) Ping(ctx context.Context, sessionID string) error {
	if a.Backend == nil {
		return fmt.Errorf("no terminal backend")
	}
	if sessionID != "" && !a.Backend.IsAlive(sessionID) {
		return fmt.Errorf("gemini pane %s not found", sessionID)
	}
	return nil
}

func (a *GeminiAdapter) Pend(ctx context.Context, sessionID string) (string, error) {
	if a.lastReply != "" {
...
func (a *GeminiAdapter) EnsurePane(ctx context.Context, workDir string) (string, error) {
	sess, err := session.LoadGeminiSession(workDir)
	if err != nil {
		return "", err
	}
	...
	return "", fmt.Errorf("no gemini session configured")
}
